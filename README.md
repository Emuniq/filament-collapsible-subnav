...
### Local Development
To use this plugin locally before publishing, add a module repository to your application's `composer.json`:

```json
"repositories": [
    {
        "type": "path",
        "url": "./packages/filament-collapsible-subnav"
    }
],
```
...
1.  **Create a Repository**: Create a new repository on GitHub (e.g., `emuniq/filament-collapsible-subnav`).
2.  **Push Code**: Initialize git inside `packages/filament-collapsible-subnav` and push to GitHub.
3.  **Packagist**: Go to [packagist.org](https://packagist.org), log in, and submit your GitHub repository URL.
4.  **Install**: Once published, anyone can install via `composer require emuniq/filament-collapsible-subnav`.
